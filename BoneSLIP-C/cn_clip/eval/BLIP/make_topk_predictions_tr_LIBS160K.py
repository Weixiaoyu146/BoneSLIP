...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        # required=True,
        default=DATAPATH + model + split + "_tr_predictions.jsonl",
        ...
        with open(args.image_feats, "r") as fin:
            for line in tqdm(fin):
                ...
                while idx < len(text_ids):
                    text_feats_tensor = torch.from_numpy(text_feats_array[idx: min(idx + args.eval_batch_size,
                                                                                   len(text_ids))]).cuda()  # [batch_size, feature_dim]
                    batch_scores = image_feat_tensor @ text_feats_tensor.t()  # [1, batch_size]
                    for text_id, score in zip(text_ids[idx: min(idx + args.eval_batch_size, len(text_ids))],
